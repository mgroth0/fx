...
abstract class XYChartForPackagePrivateProps<X, Y>(
    ...
    private val seriesChanged =
        ListChangeListener { c: Change<out Series<X, Y>?> ->
            val series = c.list
            while (c.next()) {
            ...
            override fun invalidated() {
                ...
                if (old != null || current != null) {
                    ...
                    if (toIndex > 0 || !removed.isEmpty()) {
                        seriesChanged.onChanged(
                            object : NonIterableChange<Series<X, Y>>(0, toIndex, current!!) {
                    ...
                    seriesChanged.onChanged(
                        object : NonIterableChange<Series<X, Y>?>(0, 0, current) {
    ...
    final override fun layoutChartChildren(
        top: Double,
        left: Double,
        width: Double,
        height: Double
        ...
        xAxisWidth = ceil(xAxisWidth)
        xAxisHeight = ceil(xAxisHeight)
        yAxisWidth = ceil(yAxisWidth)
        yAxisHeight = ceil(yAxisHeight)
        /* calc xAxis height */
        var xAxisY = 0.0
 ...
 update vertical grid lines
        ...
        if (isAlternativeColumnFillVisible()) {
            ...
            tickPositionsPositive.sort()
            tickPositionsNegative.sort()
     ...
     */
    internal fun getDisplayedDataIterator(
        series: Series<X, Y>
    ): Iterator<Data<X, Y>> = Collections.unmodifiableList(series.displayedData).iterator()
    ...
    class Data<X, Y> {
        ...
        internal val xValueProp: ObjectProperty<X> =
            object : SimpleObjectProperty<X>(this@Data, "XValue") {
                override fun invalidated() {
                    if (series != null) {
        ...
        val yValueProp: ObjectProperty<Y> =
            object : SimpleObjectProperty<Y>(this@Data, "YValue") {
                override fun invalidated() {
                    if (series != null) {
                        val chart = series!!.getChart()
         ...
         * for BubbleChart.
         */
        internal val extraValue: ObjectProperty<Any> =
            object : SimpleObjectProperty<Any>(this@Data, "extraValue") {
                override fun invalidated() {
                    if (series != null) {
                        val chart = series!!.getChart()
         ...
         */
        val nodeProp: ObjectProperty<Node> =
            object : SimpleObjectProperty<Node>(this, "node") {
                override fun invalidated() {
                    val node = get()
                    if (node != null) {
                        ...
                        val seriesLabel: ObservableValue<String> =
                            seriesProperty
                                .flatMap(Series<X, Y>::nameProperty)
                                .orElse("")
                        val xAxisLabel: ObservableValue<String> =
                            seriesProperty
                        ...
                        val yAxisLabel: ObservableValue<String> =
                            seriesProperty
                                .flatMap(Series<X, Y>::chartProperty)
                        ...
                        node.accessibleTextProperty().bind(
    ...
    class Series<X, Y> @JvmOverloads constructor(data: ObservableList<Data<X, Y>> = FXCollections.observableArrayList()) :
        SeriesIdea {
            ...
            internal val name: StringProperty =
                        ...
                        if (old != null || current != null) {
                            ...
                            if (toIndex > 0 || !removed.isEmpty()) {
                                dataChangeListener.onChanged(
                                    object : NonIterableChange<Data<X, Y>>(0, toIndex, current) {
                            ...
                            /* let series listener know all old series have been removed */
                            dataChangeListener.onChanged(
                                object : NonIterableChange<Data<X, Y>>(0, 0, current) {
