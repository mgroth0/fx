package matt.fx.control.wrapper.chart.line.highperf.relinechart.xy.chart
...
import matt.fx.control.wrapper.chart.line.highperf.relinechart.xy.chart.ChartForPrivateProps.StyleableProperties.classCssMetaData
...
abstract class ChartForPrivateProps: Region() {
   ...
   */
  protected abstract fun layoutChartChildren(top: Double, left: Double, width: Double, height: Double)
	...
	if (legend != null) {
	  ...
	  if (shouldShowLegend) {
		if (getLegendSide() == TOP) {
		  ...
		  val legendWidth = Utils.boundedSize(snapSizeX(legend.prefWidth(legendHeight)), 0.0, width - left - right)
		  ...
		  legend.resizeRelocate(
			left + (width - left - right - legendWidth)/2, height - bottom - legendHeight, legendWidth, legendHeight
		  )
		  if (height - bottom - top - legendHeight < MIN_HEIGHT_TO_LEAVE_FOR_CHART_CONTENT) {
			shouldShowLegend = false
		  } else {
		  ...
		  val legendHeight = Utils.boundedSize(snapSizeY(legend.prefHeight(legendWidth)), 0.0, height - top - bottom)
		  ...
		  val legendHeight = Utils.boundedSize(snapSizeY(legend.prefHeight(legendWidth)), 0.0, height - top - bottom)
		  legend.resizeRelocate(
			width - right - legendWidth, top + (height - top - bottom - legendHeight)/2, legendWidth, legendHeight
		  )
		  if (width - left - right - legendWidth < MIN_WIDTH_TO_LEAVE_FOR_CHART_CONTENT) {
			shouldShowLegend = false
		  } else {
  ...
  private object StyleableProperties {
	...
	val LEGEND_VISIBLE: CssMetaData<ChartForPrivateProps, Boolean> = object: CssMetaData<ChartForPrivateProps, Boolean>(
	  "-fx-legend-visible",
