package matt.fx.control.wrapper.tooltip.node
...
import matt.fx.control.wrapper.popupcontrol.node.bridge.MyPopUpCSSBridge
import matt.fx.control.wrapper.tooltip.node.skin.MyTooltipSkin
...
Because styling the built in matt.fx.control.wrapper.tooltip.fixed.tooltip is unsupported, requires hacks, buggy, and unreliable.
...
class MyTooltip @JvmOverloads constructor(text: String? = null): MyPopupControl() {
  /**
   * The text to display in the matt.fx.control.wrapper.tooltip.fixed.tooltip. If the text is set to null, an empty
  ...
  private val text: StringProperty = object: SimpleStringProperty(this, "text", "") {
	override fun invalidated() {
	  ...
	  if (isShowing && value != null && value != getText()) {
		//Dynamic matt.fx.control.wrapper.tooltip.fixed.tooltip content is location-dependant.
		//Chromium trick.
  ...
  /**
   * The delay between the mouse entering the hovered node and when the associated matt.fx.control.wrapper.tooltip.fixed.tooltip will be shown to the user.
  ...
  /**
   * The duration that the matt.fx.control.wrapper.tooltip.fixed.tooltip should remain showing for until it is no longer visible to the user.
   * If the mouse leaves the control before the showDuration finishes, then the matt.fx.control.wrapper.tooltip.fixed.tooltip will remain showing
  ...
  /**
   * The duration in which to continue showing the matt.fx.control.wrapper.tooltip.fixed.tooltip after the mouse has left the node. Once this time has
   * elapsed the matt.fx.control.wrapper.tooltip.fixed.tooltip will hide. The default value is 200ms.
   *
  ...
  /**
   * Typically, the matt.fx.control.wrapper.tooltip.fixed.tooltip is "activated" when the mouse moves over a Control.
  ...
  /**
   * Creates a matt.fx.control.wrapper.tooltip.fixed.tooltip with the specified text.
   *
   * @param text A text string for the matt.fx.control.wrapper.tooltip.fixed.tooltip.
   */
  /* *************************************************************************
     *                                                                         *
     * Constructors                                                            *
     ...
     **************************************************************************/
  /**
   * Creates a matt.fx.control.wrapper.tooltip.fixed.tooltip with an empty string for its text.
   */
  init {
	text?.let { setText(it) }
	bridge = MyToolTipCSSBridge(this)
	PopupWindowHelper.getContent(this).setAll(bridge)
	styleClass.setAll("matt.fx.control.wrapper.tooltip.fixed.tooltip")
  }
  ...
  private class TooltipBehavior internal constructor(private val hideOnExit: Boolean) {
	/*
         ...
         * This second timer will allow the matt.fx.control.wrapper.tooltip.fixed.tooltip to remain visible for some time
         * period (such as 5 seconds). If the mouse hasn't moved, and the HIDE_TIMER
         * expires, then the matt.fx.control.wrapper.tooltip.fixed.tooltip is hidden and the matt.fx.control.wrapper.tooltip.fixed.tooltip is no longer
         * activated.
         *
         ...
         * the HIDE_TIMER. Thus, the matt.fx.control.wrapper.tooltip.fixed.tooltip disappears after 5 seconds from the
         * last mouse move.
         *
         * If some other mouse event occurs while the HIDE_TIMER is running, other
         * than mouse move or mouse enter/exit (such as a click), then the matt.fx.control.wrapper.tooltip.fixed.tooltip
         ...
         * HIDE_TIMER and start the LEFT_TIMER, and immediately hide the matt.fx.control.wrapper.tooltip.fixed.tooltip.
         * This timer is very short, maybe about a 1/2 second. If the mouse enters a
         * new node which also has a matt.fx.control.wrapper.tooltip.fixed.tooltip before LEFT_TIMER expires, then the
         * second matt.fx.control.wrapper.tooltip.fixed.tooltip is activated and shown immediately (the ACTIVATION_TIMER
         * having been bypassed), and the HIDE_TIMER is started. If the LEFT_TIMER
         * expires and there is no mouse movement over a control with a matt.fx.control.wrapper.tooltip.fixed.tooltip,
         * then we are back to the initial steady state where the next mouse move
         * over a node with a matt.fx.control.wrapper.tooltip.fixed.tooltip installed will start the ACTIVATION_TIMER.
         */
	...
	/**
	 * The Node with a matt.fx.control.wrapper.tooltip.fixed.tooltip over which the mouse is hovering. There can
	 * only be one of these at a time.
	 */
	var hoveredNode: Node? = null

	/**
	 * The matt.fx.control.wrapper.tooltip.fixed.tooltip that is currently activated. There can only be one
	 * of these at a time.
	 */
	private var activatedTooltip: MyTooltip? = null

	/**
	 * The matt.fx.control.wrapper.tooltip.fixed.tooltip that is currently visible. There can only be one
	 * of these at a time.
	 */
	 ...
	 * show the matt.fx.control.wrapper.tooltip.fixed.tooltip (if the LEFT_TIMER is running).
	 */
	private val MOVE_HANDLER = EventHandler<MouseEvent> { event: MouseEvent ->
	  //Screen coordinates need to be actual for dynamic matt.fx.control.wrapper.tooltip.fixed.tooltip.
	  //See Tooltip.setText
	  ...
	  // with the target node and also associate the matt.fx.control.wrapper.tooltip.fixed.tooltip with the
	  ...
	  if (t != null) {
		...
		if (owner != null && treeVisible) {
		  // Now we know that the currently HOVERED node has a matt.fx.control.wrapper.tooltip.fixed.tooltip
		  // and that it is part of a visible window Hierarchy.
		  // If LEFT_TIMER is running, then we make this matt.fx.control.wrapper.tooltip.fixed.tooltip
		  ...
		  if (leftTimer.status == Animation.Status.RUNNING) {
			...
			// Force the CSS to be processed for the matt.fx.control.wrapper.tooltip.fixed.tooltip so that it uses the
			...
			// Start / restart the timer and make sure the matt.fx.control.wrapper.tooltip.fixed.tooltip
	 ...
	 * stop the HIDE_TIMER, hide the matt.fx.control.wrapper.tooltip.fixed.tooltip, and start the LEFT_TIMER.
	 */
	 ...
	 * occur, then the matt.fx.control.wrapper.tooltip.fixed.tooltip is hidden (if it is visible), it is deactivated,
	...
	init {
	  activationTimer.onFinished = EventHandler {
		...
		// If the ACTIVATED matt.fx.control.wrapper.tooltip.fixed.tooltip is part of a visible window
		// hierarchy, we can go ahead and show the matt.fx.control.wrapper.tooltip.fixed.tooltip and
		...
		// matt.fx.control.wrapper.tooltip.fixed.tooltip, and we don't start the HIDE_TIMER. We simply let
		...
		if (owner != null && owner.isShowing && treeVisible) {
		  ...
		  // The matt.fx.control.wrapper.tooltip.fixed.tooltip always inherits the nodeOrientation of
		  ...
		  // nodeOrientation on matt.fx.control.wrapper.tooltip.fixed.tooltip.getScene().getRoot().
		  ...
		  // RT-37107: Ensure the matt.fx.control.wrapper.tooltip.fixed.tooltip is displayed in a position
		  // where it will not be under the mouse, even when the matt.fx.control.wrapper.tooltip.fixed.tooltip
		  // is near the edge of the screen
		  if (y + TOOLTIP_YOFFSET > activatedTooltip!!.anchorY) {
			// the matt.fx.control.wrapper.tooltip.fixed.tooltip has been shifted vertically upwards,
		...
		// Once the activation timer has expired, the matt.fx.control.wrapper.tooltip.fixed.tooltip is no
	  ...
	  leftTimer.onFinished = EventHandler { _: ActionEvent? ->
		if (!hideOnExit) {
		  // Hide the currently visible matt.fx.control.wrapper.tooltip.fixed.tooltip.
		  assert(visibleTooltip != null)
		  visibleTooltip!!.hide()
		  visibleTooltip = null
		  hoveredNode = null
	...
	fun install(node: Node?, t: MyTooltip) {
	  ...
	  // the given node. Stash the matt.fx.control.wrapper.tooltip.fixed.tooltip in the node's client properties
  ...
  companion object {
	private const val TOOLTIP_PROP_KEY = "matt.fx.control.wrapper.matt.fx.control.wrapper.tooltip.fixed.tooltip.node.Tooltip"

	// RT-31134 : the matt.fx.control.wrapper.tooltip.fixed.tooltip style includes a shadow around the matt.fx.control.wrapper.tooltip.fixed.tooltip with a
	// width of 9 and height of 5. This causes mouse events to not reach the control
	// underneath resulting in losing hover state on the control while the matt.fx.control.wrapper.tooltip.fixed.tooltip is showing.
	// Displaying the matt.fx.control.wrapper.tooltip.fixed.tooltip at an offset indicated below resolves this issue.
	// RT-37107: The y-offset was upped to 7 to ensure no overlaps when the matt.fx.control.wrapper.tooltip.fixed.tooltip
	...
	/**
	 * Associates the given [Tooltip] with the given [Node]. The matt.fx.control.wrapper.tooltip.fixed.tooltip
	 * can then behave similar to when it is set on any [Control]. A single
	 * matt.fx.control.wrapper.tooltip.fixed.tooltip can be associated with multiple nodes.
	 * @param node the node
	 ...
	 * matt.fx.control.wrapper.tooltip.fixed.tooltip.
	 * @param node the node
	 * @param t the matt.fx.control.wrapper.tooltip.fixed.tooltip
	 * @see Tooltip
	 */
