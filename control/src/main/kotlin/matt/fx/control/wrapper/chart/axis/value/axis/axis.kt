 ...
 * questions.
 */
package matt.fx.control.wrapper.chart.axis.value.axis
...
import matt.fx.control.wrapper.chart.axis.value.axis.AxisForPackagePrivateProps.StyleableProperties.classCssMetaData
...
abstract class AxisForPackagePrivateProps<T>: Region() {
  ...
  /** The axis label  */
  private val label: ObjectProperty<String> = object: ObjectPropertyBase<String>() {
	override fun invalidated() {
	  axisLabel.text = get()
	  requestAxisLayout()
	}
  ...
  internal val tickLabelFont: ObjectProperty<Font> = object: StyleableObjectProperty<Font>(Font.font("System", 8.0)) {
	override fun invalidated() {
	  val f = get()
	  measure.font = f
  ...
  override fun computePrefHeight(width: Double): Double {
	...
	return if (side.isVertical) {
	  ...
	  val tickMarkLength: Double = if (isTickMarkVisible()) if (getTickLength() > 0) getTickLength() else 0.0 else 0.0
  ...
  private fun updateTickMarks(side: Side, length: Double) {
   ...
   */
  private fun isTickLabelsOverlap(side: Side, m1: TickMark<T>, m2: TickMark<T>, gap: Double): Boolean {
   ...
   */
  private fun positionTextNode(node: Text, posX: Double, posY: Double, angle: Double, side: Side) {
	node.layoutX = 0.0
	node.layoutY = 0.0
	node.rotate = angle
   ...
   */
  protected fun measureTickMarkLabelSize(labelText: String?, rotation: Double): Dimension2D {
   ...
   * @return size of tick mark label for given value
   */
  protected fun measureTickMarkSize(value: T, rotation: Double): Dimension2D {
   ...
   */
  private fun measureTickMarkSize(value: T, side: Side): Double {
   ...
   */
  {
	 ...
	 */
	private val label: StringProperty = object: StringPropertyBase() {
	  override fun invalidated() {
		textNode.text = value
	  }
	...
	/** true if tick mark labels should be displayed  */
	private val textVisible: BooleanProperty = object: BooleanPropertyBase(true) {
	  override fun invalidated() {
		if (!get()) {
  ...
  private object StyleableProperties {
	...
	val TICK_LENGTH: CssMetaData<AxisForPackagePrivateProps<*>, Number> = object: CssMetaData<AxisForPackagePrivateProps<*>, Number>(
	  "-fx-tick-length",
	...
	val TICK_LABEL_FONT: CssMetaData<AxisForPackagePrivateProps<*>, Font> = object: FontCssMetaData<AxisForPackagePrivateProps<*>>(
	  "-fx-tick-label-font",
	...
	val TICK_LABEL_FILL: CssMetaData<AxisForPackagePrivateProps<*>, Paint> = object: CssMetaData<AxisForPackagePrivateProps<*>, Paint>(
	  "-fx-tick-label-fill",
	...
	val TICK_LABEL_TICK_GAP: CssMetaData<AxisForPackagePrivateProps<*>, Number> = object: CssMetaData<AxisForPackagePrivateProps<*>, Number>(
	  "-fx-tick-label-gap",
	...
	val TICK_MARK_VISIBLE: CssMetaData<AxisForPackagePrivateProps<*>, Boolean> = object: CssMetaData<AxisForPackagePrivateProps<*>, Boolean>(
	...
	val TICK_LABELS_VISIBLE: CssMetaData<AxisForPackagePrivateProps<*>, Boolean> = object: CssMetaData<AxisForPackagePrivateProps<*>, Boolean>(
