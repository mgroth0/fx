...
open class ArcWrapper(
    ...
    val centerXProp = node.centerXProperty().toNonNullableProp()
    val centerYProp = node.centerYProperty().toNonNullableProp()
    var centerX by centerXProp
    var centerY by centerYProp

    val radiusXProp = node.radiusXProperty().toNonNullableProp()
    val radiusYProp = node.radiusYProperty().toNonNullableProp()

    var radiusX by node::radiusX
    var radiusY by node::radiusY
    var startAngle by node::startAngle
    var length by node::length
    var type by node::type
