...
private val FALLBACK_FILE = (ICON_FOLDER + "chunk.png").apply {
...
fun Icon(image: Image): ImageView = ImageView(image).apply {
	  ...
	  if (f.extension == "svg")
		SVGUniverse().let {
		  ...
		  val g = bim.createGraphics().apply {
			...
			/*renderingHints = RenderingHints(RENDERING_HINTS)*/
			//		renderingHints.putAll(RENDERING_HINTS)
		  }
		  dia.apply {
			...
			root.build()
			//		  println("svg string = ${this.toString()}")
