...
class MBackedFXListChange<E>(private val mChange: ListChange<E>, list: ObservableList<E>): Change<E>(list) {
  ...
  override fun getTo() = currentMChange!!.run {
	when (this) {
	  ...
	  is RemoveAtIndices       -> {
		removedElementsIndexed.zipWithNext { a, b -> require(b.index.i == a.index.i + 1) }
		lowestChangedIndex
	  }
