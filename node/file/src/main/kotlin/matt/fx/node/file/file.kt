...
fun matt.file.JioFile.draggableIcon() =
    staticIcon().apply {
...
private fun matt.file.JioFile.iconString() =
    when {
...
fun ObsVal<out FsFile>.draggableIcon() =
    ObsStringIcon(
        binding {
...
private fun matt.file.JioFile.createNodeInner(renderHTMLAndSVG: Boolean = false): RegionWrapper<NodeWrapper> {
    if (exists()) {
        ...
        return when (typed().fileType) {
            Log        ->
                VBoxWrapperImpl<NodeWrapper>().apply {
                    ...
                    textarea {
                        ...
                        var lastMod = FileTime.fromMillis(0)
            ...
            Html       ->
                WebViewPane(this@createNodeInner).apply {
                    specialZooming()
                }
            ...
            Svg        ->
                WebViewWrapper().apply {
                        ...
                        VBoxWrapperImpl<NodeWrapper>().apply {
                            mcontextmenu {
                                "refresh" does {
                                    ...
                                    /*wv.engine.document.normalizeDocument()


                                    shot in the dark, but not supported yet*/
                    ...
                    val svgWidthPx = svgText.substringAfterSingular("width=\"").substringBeforeSingular("px").toInt()
                    val svgHeightPx = svgText.substringAfterSingular("height=\"").substringBeforeSingular("px").toInt()
            ...
            FolderType ->
                when {
                    !hasExtension(APP) ->
                        fileTreeAndViewerPane(this).apply {
                            prefWidth = 600.0
                        }
